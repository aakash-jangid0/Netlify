/*
  ...
  1. Changes
    - Ensure display_order_id is populated correctly
    - Remove payment_status column from invoices table
    - Update create_invoice_from_order function to properly populate data
*/

-- Create function to extract short ID from UUID if it doesn't exist already
CREATE OR REPLACE FUNCTION get_short_id(id uuid)
RETURNS text AS $$
BEGIN
  -- Extract last 6 characters from UUID for human-readable reference
  RETURN substring(id::text, 25, 6);
END;
$$ LANGUAGE plpgsql;

-- Add display_order_id to invoices if doesn't exist
ALTER TABLE invoices 
  ADD COLUMN IF NOT EXISTS display_order_id text;

-- Create index for faster lookup by display_order_id
