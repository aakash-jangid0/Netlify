-- Create type for performance ratings if it doesn't exist
DO $$
BEGIN
...
-- Add sample data to staff_documents if empty
INSERT INTO public.staff_documents (staff_id, document_type, document_name, document_url)
SELECT s.id, 'contract', 'Employment Contract', 'https://example.com/contract.pdf'
FROM public.staff s
LEFT JOIN public.staff_documents sd ON s.id = sd.staff_id
WHERE sd.id IS NULL
LIMIT 10;

-- Fix any missing staff_id relationships
UPDATE public.staff_documents
SET staff_id = (SELECT id FROM public.staff ORDER BY created_at LIMIT 1)
